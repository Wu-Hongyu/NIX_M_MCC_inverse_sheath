...
mcc_para.n_target_H=0.2*mcc_para.n_target;%氢原子1:4的比例
mcc_para.n_target=0.8*mcc_para.n_target;%氢分子
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%电子的  H2  H
[max_secXv ,~]=max(Xsec.tot{1,1}.*sqrt(2*abs(constants.q_m_ratio_e)*Xsec.energy));
mcc_para.max_rate=mcc_para.n_target*max_secXv;%最大反应速率
mcc_para.P_null=1-exp(-mcc_para.max_rate*simulation.dt);%最大反应概率

[max_secXv_H ,~]=max(Xsec.tot{1,1}.*sqrt(2*abs(constants.q_m_ratio_e)*Xsec.energy));
