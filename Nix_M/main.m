...
simulation.num_grid_point=1601;%网格格点数目
...
Bdelta=0.01;%形状因子
% 粒子
simulation.particle_boundaries_type=2;%粒子边界条件类型 0周期 1成对+热化  2成对恒注入+热化
simulation.field_boundaries_type=0;%电位边界条件类型 0左右一类边界  3周期边界
inject_num_pertime=20;%每次注入2个电子和1个H+和1个H2+
source_density=0;
...
%气压
simulation.pressure=0.3;%气压 单位Pa
...
simulation.num0_macro_e=10000;%初始电子数目 Particle e Num
...
vHth=sqrt(constants.q_m_ration_H*simulation.THp);%离子温度对应的热速度
        ...
        case 2 %成对恒注入
            ...
            if source_density<(simulation.ne0)
                if rem(ti,100)==1
                    ...
                    position_H=[position_H; temp_matrix(1:end/2)];%合并回原来的数组
                    ...
                    position_H2p=[position_H2p; temp_matrix(end/2+1:end)];%合并回原来的数组
...
%         H2pColli_index=randperm(length(vH2p),max_collision_num);%抽取发生了空碰撞的粒子
%         
%         max_collision_num=round(0.1*mcc_para.P_null*length(vH));
%         HColli_index=randperm(length(vH),max_collision_num);%抽取发生了空碰撞的粒子
%         
